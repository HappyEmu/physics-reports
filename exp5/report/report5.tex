...
Using formula \ref{eq:omegar} and the values for alpha obtained from the graphical analysis we can again calculate $\omega_0$:
\begin{table}[H]
\center
\begin{tabular}{lcl}
...
The theoretical curves fit our expectations quite well. They are reasonably close to our measured values and additionally, they explain the discrepancies between the $\alpha$ values obtained by both methods. If we had taken the theoretical curves for the graphical analysis, we think the results would have been much closer together.
...
In order to not having to wildly guess whether or not our results are accurate, we measured the frequency of the free oscillation of the system with the eddy brake deactivated - which should be a very good approximation of an undamped oscillation - and got the result:
